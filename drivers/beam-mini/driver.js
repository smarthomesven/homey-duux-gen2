...
module.exports = class BeamMiniDriver extends Homey.Driver {

  /**
   * onInit is called when the driver is initialized.
   */
  async onInit() {
    this.log('Beam Mini driver has been initialized');
  }
  ...
  async onPair(session) {
    this.log('Beam Mini pairing started');
  ...
  async onRepair(session) {
    this.log('Neo repairing started');
  ...
  async onPairListDevices() {
    try {
      ...
      const availableDevices = allDevices
        .filter(device => device.type !== '56' && device.type !== '32' && device.type !== '23' && device.type !== '21' && device.type !== '50' && device.type !== '26')
        .map(device => ({
