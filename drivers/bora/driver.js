...
module.exports = class BoraDriver extends Homey.Driver {

  /**
   * onInit is called when the driver is initialized.
   */
  async onInit() {
    this.log('Bora driver has been initialized');
  }
  ...
  async onRepair(session) {
    this.log('Bora repairing started');
    this._type = "repair";
    this._session = session;
  ...
  async onPairListDevices() {
    try {
      ...
      const availableDevices = allDevices
        .filter(device => device.type === "56" || device.type === "53")
        .map(device => ({
